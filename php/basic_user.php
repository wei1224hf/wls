...
class basic_user {
	...
	public static function checkPermission($executor, $action=FALSE, $session=FALSE){
		return true;//TODO
	...
	public static function modify($data=NULL,$executor=NULL){   
		...
		$str_keys = ",username,group_code,status,type,password,money,credits,govern_zone,";		
		$sql = "";
	...
	public static function add($data=NULL,$executor=NULL){
		...
		$id = tools::getTableId("basic_user");
		$item['id'] = $id;
		$item["creater_id"] = $session["data"]["user_id"];
    	$item["updater_id"] = $session["data"]["user_id"];
    	$item["creater_group_code"] = $session["data"]["group_code"];
    	$item["time_created"] = date("Y-m-d");    
    	$item["time_lastupdated"] = date("Y-m-d");
    	$item["remark"] = "excel upload";

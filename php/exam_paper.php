...
class exam_paper {
	...
	public static function checkMyAnswers($myAnswers,$paper_id){
        ...
        while($temp = mysql_fetch_assoc($res)){
        	...
        	if($temp['type']=="1"||$temp['type']=="2"||$temp['type']=="3"){
        		...
        		$temp['result']=2;
        		$t_return['result']['mycent_subjective'] +=$temp['cent'];        		
        	}else{
        		$temp['result']=3;
        	}
        ...
        $t_return['result']['mycent_objective'] = $t_return['result']['mycent'];
	...
	public static function upload($file,$executor){  
        ...
        $data__exam_paper = array(
            ...
            ,'title'=>trim($currentSheet->getCell('B2')->getValue())
            ,'cost'=>trim($currentSheet->getCell('C2')->getValue())
            ,'type'=>10
            ,'status'=>10
            ,'cent'=>0
            ,'cent_subjective'=>0
        ...
        for($i=2;$i<=$row;$i++){
            ...
            if($index_==NULL || !is_numeric($index_) || ($index_*1)!=$index ){
            	return array(
            			'msg'=>tools::$LANG['basic_normal']['cellError'].": A".$i." :".$index_
            			,'status'=>'2'
            	);
            }
            ...
            $cent = trim($currentSheet->getCell('C'.$i)->getValue());
            if($cent==NULL){
            	$cent = 0;
            }else{
            	if(!is_numeric($cent)){
            ...
            $type = trim($currentSheet->getCell('E'.$i)->getValue());
            ...
            $exam_question__subject = trim($currentSheet->getCell('F'.$i)->getValue());
            ...
            $title = trim($currentSheet->getCell('G'.$i)->getValue());
            $title = exam_paper::formatStr($title); 
            
            $option_length = trim($currentSheet->getCell('H'.$i)->getValue());
            ...
            $option_1 = trim($currentSheet->getCell('I'.$i)->getValue());
            $option_1 = exam_paper::formatStr($option_1);
            $option_2 = trim($currentSheet->getCell('J'.$i)->getValue());
            $option_2 = exam_paper::formatStr($option_2);
            $option_3 = trim($currentSheet->getCell('K'.$i)->getValue());
            $option_3 = exam_paper::formatStr($option_3);
            $option_4 = trim($currentSheet->getCell('L'.$i)->getValue());
            $option_4 = exam_paper::formatStr($option_4);
            $option_5 = trim($currentSheet->getCell('M'.$i)->getValue());
            $option_5 = exam_paper::formatStr($option_5);
            $option_6 = trim($currentSheet->getCell('N'.$i)->getValue());
            $option_6 = exam_paper::formatStr($option_6);
            $option_7 = trim($currentSheet->getCell('O'.$i)->getValue());
            $option_7 = exam_paper::formatStr($option_7);
            
            $answer = trim($currentSheet->getCell('P'.$i)->getValue());
            $answer = exam_paper::formatStr($answer);       
            $description = trim($currentSheet->getCell('Q'.$i)->getValue());
            ...
            $data__exam_question = array(
                ...
                ,'path_listen'=>trim($currentSheet->getCell('T'.$i)->getValue())
                ,'path_img'=>trim($currentSheet->getCell('U'.$i)->getValue())
